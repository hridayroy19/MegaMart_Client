...
const ShopingSidebar: React.FC<ShopingSidebarProps> = ({
    ...
    return (
        <aside className={`w-full bg-background p-5 space-y-8 h-full  overflow-y-auto
            ...
            <div>
                ...
                <ul className={`space-y-3 max-h-[650px] overflow-y-auto pr-2 ${styles.customScrollbar}`}>
                    {categories.map((cat, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`text-xs px-2 py-1 rounded-full ${isActive ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-500 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
                                    {cat.count}
                                </span>
                            </li>
                        )
            ...
            <div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-gray-600">
                        Price: <span className="text-foreground font-bold">$0 - ${localMaxPrice}</span>
                    </span>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {ratings.map((item, idx) => {
                        ...
                        return (
                            <div key={idx} onClick={() => handleRatingClick(item.stars)} className="flex items-center gap-3 text-sm cursor-pointer group">
                                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isActive ? 'border-blue-600' : 'border-gray-300 group-hover:border-blue-600'}`}>
                                    <div className={`w-2.5 h-2.5 rounded-full ${isActive ? 'bg-blue-600' : 'bg-transparent group-hover:bg-blue-100'}`}></div>
                                </div>
                                <div className="flex items-center gap-0.5 text-orange-400">
                                    {[...Array(5)].map((_, i) => (
                                        <Star key={i} size={14} className={i < item.stars ? "fill-orange-400" : "text-gray-300 fill-gray-200"} />
                                    ))}
                                </div>
                                ...
                                <span className={`w-8 text-right text-xs font-medium ${isActive ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600'}`}>
                                    {item.count}
                                </span>
                            </div>
                        )
            ...
            <div>
                ...
                <ul className="space-y-3">
                    {colors.map((item, idx) => {
                        ...
                        return (
                            <li key={idx} onClick={() => toggleColor(item.name)} className="flex items-center gap-3 cursor-pointer group">
                                <div
                                    ...
                                    style={{ backgroundColor: item.name !== 'White' ? item.color : undefined }}
                                ></div>
                                <span className={`text-sm transition-colors ${isActive ? 'text-blue-600 font-medium' : 'text-gray-600 group-hover:text-blue-600'}`}>
            ...
            <div>
                ...
                <ul className="space-y-3">
                    {brands.map((brand, idx) => {
                        ...
                        return (
                            <li key={idx} onClick={() => toggleBrand(brand)} className="flex items-center gap-3 cursor-pointer group">
                                <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${isActive ? 'border-blue-600 bg-blue-600' : 'border-gray-300 group-hover:border-blue-600'}`}></div>
                                <span className={`text-sm transition-colors ${isActive ? 'text-blue-600 font-medium' : 'text-gray-600 group-hover:text-blue-600'}`}>
                                    {brand}
                                </span>
                            </li>
            ...
            <div className="w-full pt-2">
                <img src={typeof advertise === 'string' ? advertise : (advertise as any).src} alt="Special Offer" className="w-full h-auto rounded-lg object-cover hover:opacity-95 transition-opacity cursor-pointer" />
            </div>
